  ...
  return (
    <div className='cursor-pointer sm:p-3 sm:hover:shadow-slate-400 sm:shadow-md rounded-lg sm:border dark:hover:shadow-slate-500 sm:border-slate-400 sm:m-2 transition-shadow duration-200 group'>
      <Link href={`/movie/${result.id}`}>
        <Image src={`https://image.tmdb.org/t/p/original/${result.backdrop_path || result.poster_path || imageNotFound}`} width={500} style={{maxWidth: "100%", height: "auto"}}className='sm:rounded-t-lg group-hover:opacity-80 transition-opacity object-cover duration-200' height={300} placeholder='blur' blurDataURL='/Rolling-1s-200px (1).svg' alt='image not found'/>
        <div className='p-2'>
